...
class DDPG(object):
    ...
    def __init__(self, tasks_num, task_dim, vms_num, vm_dim):
        ...
        ''''''
        self.s_dim = self.tasks_num * self.task_dim + self.vms_num * self.vm_dim + 6  # 状态维度 
        self.s_task_dim = self.tasks_num * self.task_dim + 6#这两处做修改
...
class ANet(nn.Module):  # 通过 s 预测出 a
    def __init__(self, s_task_dim, s_vm_dim, a_dim):  # s_dim分割为 cut（虚拟机状态）和 s_dim-cut（任务状态）
        ...
        self.embedding1 = nn.Embedding(1000,1)
        self.embedding2 = nn.Embedding(10000,1)
        #self.embedding3 = nn.Embedding(100,1)
        self.attention_dim = 64*3 + 6
        self.query = nn.Linear(self.attention_dim,128)
        self.key = nn.Linear(self.attention_dim,128)
        self.value = nn.Linear(self.attention_dim,128)
        self.layer1_task = nn.Sequential(  # task状态的嵌入层
            nn.Conv1d(s_task_dim-6, 128,1),
            nn.BatchNorm1d(128),
            nn.LeakyReLU(),
        )
        self.layer1_vm = nn.Sequential(  # vm状态的嵌入层
            nn.Conv1d(s_vm_dim, 64,1),
            nn.BatchNorm1d(64),
            nn.LeakyReLU(),
        )
        self.layer2 = nn.Sequential(
            nn.Linear(64 * 3 + 6, 128),
    ...
    def forward(self, x):     
        x_task,x_cor, x_vm = x.split([self.s_task_dim-6,6, self.s_vm_dim], dim=-1)  # 分割
        x_task = x_task.T    
        #print(x_task.size())
        task = torch.LongTensor(x_task.numpy())
        x_task = self.embedding1(task)
        #print(x_task.size())
        x_task = x_task.permute(2,0,1)
        #print(x_task.size())
        x_vm = x_vm.T
        #x_vm = x_vm.unsqueeze(0)
        vm = torch.LongTensor(x_vm.numpy())
        x_vm = self.embedding2(vm)
        x_vm = x_vm.permute(2,0,1)
        #print(x_cor.shape)
        x_cor = torch.LongTensor(x_cor.numpy())

        #########
        #x_cor= self.embedding3(x_cor)
        #x_cor = x_cor.permute(2,0,1)
        #x_cor = x_cor.squeeze(0)
        #x_cor = x_cor.T
        #########
        #print(x_cor.size())
        ...
        #print(x_task.shape)
        #print(x_cor.shape)
        x = torch.cat([x_task,x_cor, x_vm], dim=1)  # 嵌入后拼接，dim=-1表示按列拼接
        
        mat_q = self.query(x)
        mat_k = self.key(x)
        mat_v = self.value(x)
        score = torch.mm(mat_q,mat_k.T)/(128**0.5)
        score = F.softmax(score)
        #print(score.shape)
        x = torch.mm(score,mat_v)
        x = self.layer22(x)
        x = self.layer3(x)
        x = self.layer4(x)
        return x
...
class CNet(nn.Module):  # 通过 s,a 预测出 q
    def __init__(self, s_dim, a_dim):
        ...
        self.ins = nn.Conv1d(s_dim, 128,1, bias=True)
        self.ina = nn.Conv1d(a_dim, 64,1, bias=True)

        self.attention_dim = 192
        self.query = nn.Linear(self.attention_dim,128)
        self.key = nn.Linear(self.attention_dim,128)
        self.value = nn.Linear(self.attention_dim,128)

        self.embedding1 = nn.Embedding(10000,1)

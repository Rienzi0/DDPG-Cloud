...
def main(vmsNum, interval, file_input, file_output):
    ...
    for task in result:
        ...
        mi_response_time = task[2] / task[-1]   #mi / response_time
    ...
    analyze_result.strToFile("总成本：" + str(total_cost/interval ), 'a')
    ...
    print("机器cpu利用率标准差：", np.std(cpu_utilization))
    print("机器cpu利用率:", cpu_utilization)
    print("机器完成任务数:", machines_task_num)
    analyze_result.strToFile("机器cpu利用率标准差：" + str(np.std(cpu_utilization)), 'a')
    analyze_result.strToFile("机器cpu利用率：" + str(cpu_utilization), 'a')
    analyze_result.strToFile("机器cpu利用率均值：" + str(np.mean(cpu_utilization)), 'a')
    analyze_result.strToFile("机器完成任务数：" + str(machines_task_num) + "\n", 'a')


if __name__ == '__main__':
    vmsNum = 20
    interval = 600  # 每500时间段计算一次值，比如1~499为第一段，500~999为第二段

    #file_output = "new_real_a/b=0.9(5-50)(100).txt"
    file_output = "new_real_a/b=0.9(google).txt"
    #file_output = "/Users/rienzi/Desktop/实验代码/Pycloudsim/data/another_r/pcc20(high).txt"
    ...
    for name in txtname:
        for scheduler in scheduler_li:
            ...
            #file_input = "new_real/b=0.9(5-50)(100).txt"
            file_input = "new_real/b=0.9(google).txt"
            #file_input = "/Users/rienzi/Desktop/实验代码/Pycloudsim/new_methods/pcc20(r=0.1)_50.txt"

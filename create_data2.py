...
def create_tasks(poisson_lam):
    task_num = 4000000  # 生成100万条任务
    task_mi = 150  # 长度平均值
    task_cpu_utilization = 0.6  # cpu利用率平均数
    task_data_size = 100  # 任务数据大小

    num_per_second = np.random.poisson(lam=poisson_lam, size=86400)  # lam单位时间随机时间发生次数的平均值
    task_time = []  # 存储每条任务的提交时间,从1开始
    for i, num in enumerate(num_per_second):
        if num > (poisson_lam ):  # 对每秒提交的任务数量进行限制
            num = poisson_lam 
        elif num < max(1, poisson_lam * 0.5):
            num = max(1, poisson_lam * 0.5)
            num = int(num)
        for j in range(num):
            task_time.append(i + 1)
    mi = list(map(int, np.random.normal(loc=task_mi, scale=50, size=task_num)))  # 均值mean,标准差std,数量
...
if __name__ == '__main__':
    poisson_lam_li = [50]  # 1, 3, 5, 7, 9
    for i in poisson_lam_li:
        
        poisson_lam = i  # 表示平均每秒提交任务的个数
        all_tasks = create_tasks(poisson_lam)
        print("任务生成成功！")
        result = FileIo("/Users/rienzi/Desktop/实验代码/Pycloudsim/data/create/create_tasks_l_" + str(poisson_lam) + ".txt").twoListToFile(all_tasks, 'w')
